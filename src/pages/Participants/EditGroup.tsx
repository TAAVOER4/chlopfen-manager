...
const EditGroup = () => {
  ...
  useEffect(() => {
    ...
    const autoGeneratedName = generateGroupName(groupParticipants);
    setIsNameAutoGenerated(foundGroup.name === autoGeneratedName);
  ...
  const removeParticipant = (participant: Participant) => {
    ...
    if (isNameAutoGenerated && updatedParticipants.length > 0) {
      const generatedName = generateGroupName(updatedParticipants);
      form.setValue('name', generatedName);
    }
  };
              ...
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2">
                {availableParticipants.map((participant) => {
                  ...
                  const existingGroups = participant.groupIds?.map(gId => {
                    if (currentGroup && gId === currentGroup.id) return null;
