...
export const useGroupForm = ({ 
  ...
  useEffect(() => {
    ...
    if (category === 'kids_juniors') {
      filtered = mockParticipants.filter(
        (p) => p.category === 'kids' || p.category === 'juniors'
      );
    } else {
      filtered = mockParticipants.filter(
        (p) => p.category === 'active'
  ...
  const addParticipant = (participant: Participant) => {
    ...
    // Check if participant category matches the group category
    const participantGroupCategory = mapToGroupCategory(participant.category);
    if (participantGroupCategory !== selectedCategory) {
      toast({
        title: "Falsche Kategorie",
        description: `${participant.firstName} ${participant.lastName} gehört zur Kategorie ${participantGroupCategory} und kann nicht zu einer ${selectedCategory}-Gruppe hinzugefügt werden.`,
        variant: "destructive"
      });
      return;
    }
